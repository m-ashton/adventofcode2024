...
def part2
  disk, metadata = initialize_disk_part_2('./sample.txt')

  puts metadata.inspect
  metadata[:free_space].keys.sort.each do |position|
    # FIXME infinite loop once we're done
    until position.nil?
      metadata[:files].reject { |k, v| v[:compacted] }.keys.sort.reverse.each do |file_id|
        if metadata[:files][file_id][:size] <= metadata[:free_space][position][:size]
          position = move_file(disk, metadata, id: file_id, to: position)
          puts disk.inspect
          break
        end
      end
